...
from deeplearning.coco_utils import sample_coco_minibatch, load_coco_data, decode_captions
import nltk
import pickle

def save_model_prediction(model, data):
    """
...
class CaptioningSolver(object):
    ...
    def check_accuracy(self, X, y, num_samples=None, batch_size=100):
        ...
        for i in range(num_batches):
            ...
            bleu_score = BLEU_score(y, X[start:end])
        ...
        mean_bleu = np.array(BLEUs).mean()

        return acc, mean_bleu
    ...
    def train(self):
        ...
        for t in range(num_iterations):
            ...
            if t % 200 == 0 and t != 0:
              b_score = evaluate_model(self.model, self.data)
              if b_score > self.best_score:
                self.best_score = b_score
                save_model_prediction(self.model, self.data)
                if b_score > 0.4:
                  break
