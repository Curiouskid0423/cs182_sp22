...
def rmsprop(x, dx, config=None):
    ...
    #############################################################################
    second_moment = config['decay_rate'] * config['cache'] + (1 - config['decay_rate']) * (dx**2)
    rms, eps = np.sqrt(second_moment), config['epsilon']
    next_x = x - config['learning_rate'] * dx / (rms + eps)
...
def adam(x, dx, config=None):
    ...
    """ Scale the moments for better performance in early training """
    scaled_fm = first_moment / (1 - config['beta1'] ** curr_t + eps)
    scaled_sm = second_moment / (1 - config['beta2'] ** curr_t + eps)

    next_x = x - config['learning_rate'] * scaled_fm / (np.sqrt(scaled_sm) + eps)
