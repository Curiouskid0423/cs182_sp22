...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        embedding_size = sin_embedding.size()
        position_shape = (1, embedding_size[1], embedding_size[-1]*2) # fill in the other two dimensions
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):
        ...
        attn_input = (norm_target_selfattn, norm_encoder_outputs) # (query, memories)
        encdec_attention = self.cross_attention(attn_input, mask=cross_attention_mask)
        # Take the residual between the output and the unnormalized target input of the cross-attention
        res_encdec_attention = self.feed_forward(res_target_self_attn + encdec_attention)

        output = res_encdec_attention
